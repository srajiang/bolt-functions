...
app.view('reverse', async ({ ack, body, view, client, logger }) => {
  ...
  // normalize inputs from view submission into format function expects
  const inputs = { 
    stringToReverse: view['state']['values']['stringToReverse']['stringToReverse'].value,
    channel_id: view['state']['values']['channel_id']['channel_id'].selected_channel
  };
  console.log(inputs);

  const reverseString = await reverseFunc(inputs, client);

});

const reverseFunc = async function(inputs, client) {
  const reverseString = inputs.stringToReverse.split("").reverse().join("");
  try {
    await client.chat.postMessage({
      channel: inputs.channel_id,
      text: `You reversed "${inputs.stringToReverse}" to ${reverseString}`
    })
  }
  catch (error) {
    console.error(error);
...
app.event('function_executed', async ({ event, body, client, success, error}) => {
    ...
    const reverseString =  await reverseFunc(event.inputs, client)
